...
public class HystrixMetricsPoller
{
	 ...
	 * @param delay
	 */
	public HystrixMetricsPoller(final MetricsAsJsonPollerListener listener, final int delay)
	{
	...
	{
		// use compareAndSet to make sure it starts only once and when not
		// running
	...
	/**
	 * Pause (stop) polling. Polling can be started again with <code>start</code> as long as <code>shutdown</code> is not
	 * called.
	 */
	...
	/**
	 * Used to protect against leaking ExecutorServices and threads if this class is abandoned for GC without shutting
	 * down.
	 */
	...
	private final Object finalizerGuardian = new Object()
	{
		...
		protected void finalize() throws Throwable
		{
			...
			{
				logger.warn(HystrixMetricsPoller.class.getSimpleName()
						+ " was not shutdown. Caught in Finalize Guardian and shutting down.");
				try
				{
					shutdown();
				}
	...
	private class MetricsPoller implements Runnable
	{
		...
		public MetricsPoller(final MetricsAsJsonPollerListener listener)
		{
			this.listener = listener;
		}
		...
		public void run()
		{
			try
			{
				for (final HystrixCommandMetrics commandMetrics : HystrixCommandMetrics.getInstances())
				{
					final String jsonString = getCommandJson(commandMetrics);
					listener.handleJsonMetric(jsonString);
				}

				for (final HystrixThreadPoolMetrics threadPoolMetrics : HystrixThreadPoolMetrics.getInstances())
				{
					if (hasExecutedCommandsOnThread(threadPoolMetrics))
					{
						final String jsonString = getThreadPoolJson(threadPoolMetrics);
						listener.handleJsonMetric(jsonString);
					}
				}

				for (final HystrixCollapserMetrics collapserMetrics : HystrixCollapserMetrics.getInstances())
				{
					final String jsonString = getCollapserJson(collapserMetrics);
					listener.handleJsonMetric(jsonString);
				}
		...
		{
			final HystrixCommandKey key = commandMetrics.getCommandKey();
			final HystrixCircuitBreaker circuitBreaker = HystrixCircuitBreaker.Factory.getInstance(key);
			...
			final HealthCounts healthCounts = commandMetrics.getHealthCounts();
			...
			json.writeNumberField("rollingCountFallbackEmit",
					commandMetrics.getRollingCount(HystrixRollingNumberEvent.FALLBACK_EMIT));
			...
			// property values for reporting what is actually seen by the
			// command rather than what was set somewhere
			final HystrixCommandProperties commandProperties = commandMetrics.getProperties();

			json.writeNumberField("propertyValue_circuitBreakerRequestVolumeThreshold", commandProperties
					.circuitBreakerRequestVolumeThreshold().get());
			json.writeNumberField("propertyValue_circuitBreakerSleepWindowInMilliseconds", commandProperties
					.circuitBreakerSleepWindowInMilliseconds().get());
			json.writeNumberField("propertyValue_circuitBreakerErrorThresholdPercentage", commandProperties
			...
			json.writeStringField("propertyValue_executionIsolationStrategy", commandProperties.executionIsolationStrategy().get()
					.name());
			json.writeNumberField("propertyValue_executionIsolationThreadTimeoutInMilliseconds", commandProperties
					.executionTimeoutInMilliseconds().get());
			json.writeNumberField("propertyValue_executionTimeoutInMilliseconds", commandProperties.executionTimeoutInMilliseconds()
					.get());
			json.writeBooleanField("propertyValue_executionIsolationThreadInterruptOnTimeout", commandProperties
					.executionIsolationThreadInterruptOnTimeout().get());
			json.writeStringField("propertyValue_executionIsolationThreadPoolKeyOverride", commandProperties
					.executionIsolationThreadPoolKeyOverride().get());
			json.writeNumberField("propertyValue_executionIsolationSemaphoreMaxConcurrentRequests", commandProperties
					.executionIsolationSemaphoreMaxConcurrentRequests().get());
			json.writeNumberField("propertyValue_fallbackIsolationSemaphoreMaxConcurrentRequests", commandProperties
					.fallbackIsolationSemaphoreMaxConcurrentRequests().get());

			/*
			 * The following are commented out as these rarely change and are verbose for streaming for something people
			 * don't change. We could perhaps allow a property or request argument to include these.
			 */
			...
			json.writeNumberField("propertyValue_metricsRollingStatisticalWindowInMilliseconds", commandProperties
			...
			json.writeNumberField("reportingHosts", 1); // this will get summed
			// across all instances
			// in a cluster
			json.writeStringField("threadPool", commandMetrics.getCommandKey().name());
		...
		private boolean hasExecutedCommandsOnThread(final HystrixThreadPoolMetrics threadPoolMetrics)
		{
		...
		{
			final HystrixThreadPoolKey key = threadPoolMetrics.getThreadPoolKey();
			final StringWriter jsonString = new StringWriter();
			final JsonGenerator json = jsonFactory.createJsonGenerator(jsonString);
			json.writeStartObject();
			...
			json.writeNumberField("rollingCountThreadsExecuted",
					threadPoolMetrics.getRollingCount(HystrixRollingNumberEvent.THREAD_EXECUTION));
			json.writeNumberField("rollingMaxActiveThreads", threadPoolMetrics.getRollingMaxActiveThreads());
			json.writeNumberField("rollingCountCommandRejections",
					threadPoolMetrics.getRollingCount(HystrixRollingNumberEvent.THREAD_POOL_REJECTED));

			json.writeNumberField("propertyValue_queueSizeRejectionThreshold", threadPoolMetrics.getProperties()
					.queueSizeRejectionThreshold().get());
			json.writeNumberField("propertyValue_metricsRollingStatisticalWindowInMilliseconds", threadPoolMetrics.getProperties()
					.metricsRollingStatisticalWindowInMilliseconds().get());
		...
		{
			final HystrixCollapserKey key = collapserMetrics.getCollapserKey();
			final StringWriter jsonString = new StringWriter();
			final JsonGenerator json = jsonFactory.createJsonGenerator(jsonString);
			json.writeStartObject();
			...
			json.writeNumberField("rollingCountRequestsBatched",
					collapserMetrics.getRollingCount(HystrixRollingNumberEvent.COLLAPSER_REQUEST_BATCHED));
			json.writeNumberField("rollingCountBatches", collapserMetrics.getRollingCount(HystrixRollingNumberEvent.COLLAPSER_BATCH));
			json.writeNumberField("rollingCountResponsesFromCache",
					collapserMetrics.getRollingCount(HystrixRollingNumberEvent.RESPONSE_FROM_CACHE));
			...
			// json.writeNumberField("shardSize_mean",
			// collapserMetrics.getShardSizeMean());
			// json.writeObjectFieldStart("shardSize");
			// json.writeNumberField("25",
			// collapserMetrics.getShardSizePercentile(25));
			// json.writeNumberField("50",
			// collapserMetrics.getShardSizePercentile(50));
			// json.writeNumberField("75",
			// collapserMetrics.getShardSizePercentile(75));
			// json.writeNumberField("90",
			// collapserMetrics.getShardSizePercentile(90));
			// json.writeNumberField("95",
			// collapserMetrics.getShardSizePercentile(95));
			// json.writeNumberField("99",
			// collapserMetrics.getShardSizePercentile(99));
			// json.writeNumberField("99.5",
			// collapserMetrics.getShardSizePercentile(99.5));
			// json.writeNumberField("100",
			// collapserMetrics.getShardSizePercentile(100));
			// json.writeEndObject();
			...
			json.writeNumberField("propertyValue_timerDelayInMilliseconds", collapserMetrics.getProperties()
					.timerDelayInMilliseconds().get());
